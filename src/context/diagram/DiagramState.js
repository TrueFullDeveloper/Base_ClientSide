...
export const DiagramState = ({ children }) => {
  const initialState = {
    digramAngles: [],
    isExpand: false,
  };
  ...
  const drawDiagram = (diagramRef, queryData) => {
  ...
  const diagramExpand = (diagramRef, x, y) => {
    // Circle Validation
    if (
    ...
    const sectorNumber = getSectorByCoordinates(x, y, state.digramAngles);

    const CORRECT_ANGLE = 6 * (Math.PI / 180);

    // Arcs of a Circle

    state.digramAngles.map((sector, index) => {
      diagramRef.current.fillStyle = DIAGRAM_COLORS[index];
  ...
  return (
    <DiagramContext.Provider
      value={{
        ...
        diagramExpand,
        drawDiagram,
        digramAngles: state.digramAngles,
